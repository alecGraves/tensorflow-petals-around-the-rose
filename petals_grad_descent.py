...
train_step = tf.train.GradientDescentOptimizer(1e-8).minimize(cross_entropy)

sess.run(tf.initialize_all_variables())

# Evaluation of the model:

correct_prediction = tf.greater(0.1, tf.abs(tf.sub((y), (y_))))
...
for i in range(100000):
    ...
    if (i%1000 == 0):
        print(sess.run(W, train_step.run(feed_dict={x: batchx, y_:batchy})))
    print("Accuracy: ", sess.run(accuracy, feed_dict={x: batchx, y_: batchy}))
